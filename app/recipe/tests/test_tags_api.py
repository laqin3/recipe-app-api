...
class PrivateTagsApiTests(TestCase):
    ...
    def test_tags_limited_to_user(self):
        ...
        # if we add 'user' in TagSerializer's fields
        # self.assertEqual(res.data[0]['user'], self.user.id)
        # above one fails, coz we didn't add
    ...
    def test_filter_tags_assigned_to_Recipe(self):
        """Test listing tags to those assingned to recipes. """
        tag1 = Tag.objects.create(user=self.user, name='Breakfast')
        tag2 = Tag.objects.create(user=self.user, name='Lunce')
